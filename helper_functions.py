    ...
    try:
        while True:
            ...
            if name == "" or name.isdigit():
                print("Name must only include characters and cannot be empty!")
            else:
                break
        ...
        while True:
            ...
            if age.isnumeric() == False:
                ...
                age = int(age)
                if age <= 0:
                    print("Age cannot be a negative number or cannot be zero")
                elif age > 200:
        ...
        while True:
            ...
            location = ", ".join([street, city, state])
            if location == "":
        ...
        while True:    
            ...
            if phone_number == "" or len(phone_number) != 11:
                print("Phone number cannot be empty and must be 11 digits!")
            else:
...
def update_profile(file_path, id):
    with open(file_path, 'r', encoding = 'utf-8') as f:
        user_details = json.load(f)
    particular_user = list(filter(lambda x: x['ID'] == id, user_details))
    index = user_details.index(particular_user[0])
    while True:
        name = input("Enter your name:")
        if name == "" or name.isdigit():
            print("Name must only include characters and cannot be empty!")
        else:
            particular_user[0].update({'name': name})
            break
    ...
    while True:
            ...
            if age.isnumeric() == False:
                ...
                if age <= 0:
                    ...
                    particular_user[0].update({'age': age})
                    break
    ...
    while True:
            ...
            if gender.lower() != "female" and gender.lower() != "male":
                ...
                gender = gender.title()
                particular_user[0].update({'gender': gender})
                break
    ...
    while True:
            ...
            if location == "":
                ...
                particular_user[0].update({'location': location})
                break
    ...
    while True:    
            ...
            if phone_number == "" or len(phone_number) != 11:
                ...
                particular_user[0].update({'location': location})
                user_details[index] = particular_user[0]
                break
...
def order_products(file_path, name):
    ...
    prod_no = int(input('Enter quantity here '))
    tar = [p for p in prod_list if int(p['quantity'].split(' ')[0]) >= prod_no]
    if len(tar) == 0:
        print('Products not available at that quantity')
    else:
